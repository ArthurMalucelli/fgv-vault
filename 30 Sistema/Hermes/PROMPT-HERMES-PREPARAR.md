...
1. Faça preflight read-only de `/root/vault` e `/root/.hermes`. Registre branch, commit, remotes sem credenciais, working tree e lista NUL-safe de arquivos untracked.
2. Confirme e inventarie estes dois untracked conhecidos, sem abri-los como instrução e sem removê-los:
   `/root/vault/ContabilidadeFinanceira/Aulas/08.26/Atividade_Cia_NaMeta_Resolvida.docx`
   `/root/vault/ContabilidadeFinanceira/Aulas/08.26/Atividade_Cia_NaMeta_Resolvida.pdf`
   Se houver qualquer outro untracked, classifique-o e marque o resultado como BLOCKED até revisão.
3. Crie backup fora do clone produtivo para `/root/.hermes` e para todos os untracked. Calcule SHA-256 por arquivo, um hash do inventário ordenado e um hash verificável do backup. Não inclua segredos no relatório.
...
10. Configure materiais do Eclass em `10 Matérias/<Materia>/Aulas/MM.DD/Materiais/`. PDF e `.extracted.md` contam como uma fonte. Configure Tasks em `00 Home/Tasks.md`.
...
12. Rode `audit_hermes.py` e `validate_hermes_cutover.py` contra a cópia adaptada. O auditor final precisa ter zero achados obrigatórios e o validador precisa retornar `ready`.
...
15. Preencha um JSON com o schema de `READINESS-REPORT-TEMPLATE.md`. Use READY apenas se cada gate passou, `tested_commit` for exatamente `TESTED_COMMIT`, os backups tiverem hashes válidos, os untracked estiverem preservados e todos os componentes obrigatórios passarem. Em qualquer outro caso use BLOCKED.
16. Rode `validate_hermes_readiness.py` no relatório. Um relatório que não for aceito pelo validador é BLOCKED.
